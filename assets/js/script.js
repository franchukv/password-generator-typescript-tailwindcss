...
const handleCopyButtonClick = () => {
    const input = document.querySelector('#passgen-main-input');
    const button = document.querySelector('#passgen-copy-button');
    if (!input || !button) {
        return;
    }
...
const handleTypicalRangeInput = () => {
    ...
    fields.forEach((field) => {
        ...
        const input = field.querySelector('.typical-range__input');
        if (!output || !input) {
            return;
        }
        if (input.value === '0') {
...
const handlePassgenFormSubmit = () => {
    const form = document.querySelector('#passgen-form');
    if (!form) {
        return;
    }
    const mainInput = form.querySelector('#passgen-main-input');
    const lengthRange = form.querySelector('#passgen-length');
    const checkboxes = [
        form.querySelector('#passgen-uppercase'),
        form.querySelector('#passgen-lowercase'),
        form.querySelector('#passgen-numbers'),
        form.querySelector('#passgen-symbols'),
    ];
    ...
    submitButton.addEventListener('click', (event) => {
        event.preventDefault();
        const hasChecked = checkboxes.some((checkbox) => checkbox === null || checkbox === void 0 ? void 0 : checkbox.checked);
